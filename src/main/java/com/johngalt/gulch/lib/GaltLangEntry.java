...
public class GaltLangEntry
{
    ...
    private static String getNiceName(String unwrappedUnlocalizedName)
    {
        ...
        {
            if (unwrappedUnlocalizedName.startsWith("Galt"))
                unwrappedUnlocalizedName = unwrappedUnlocalizedName.replaceFirst("Galt", "");

            if (unwrappedUnlocalizedName.startsWith("Item"))
                unwrappedUnlocalizedName = unwrappedUnlocalizedName.replaceFirst("Item", "");

            if (unwrappedUnlocalizedName.startsWith("Block"))
                unwrappedUnlocalizedName = unwrappedUnlocalizedName.replaceFirst("Block", "");
        }
        ...
        if (unwrappedUnlocalizedName.startsWith(" ")) unwrappedUnlocalizedName = unwrappedUnlocalizedName.substring(1);
    ...
    {
        return item.getUnlocalizedName().equals(_UnlocalizedName);
    }

    public String CreateLangFileLine()
    {
        return _UnlocalizedName + (_UnlocalizedName.endsWith("Tab") ? "" : ".name") + "=" + _Name;
